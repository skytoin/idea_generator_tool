...
import { FIELD_COVERAGE, type Consumer } from '../../lib/types/field-coverage';
...
TARGET_SOURCES — CRITICAL: For tech_scout.target_sources, INCLUDE ALL currently implemented sources by default: ["hn", "arxiv", "github", "reddit", "huggingface", "cloudflare"]. Only narrow the list if the founder's profile makes one specific source clearly irrelevant (rare). Each source covers a distinct slice — HN is community buzz and product launches, arxiv is research frontier, GitHub is code adoption, Reddit is niche-community voice and real-user pain complaints, Hugging Face is trending AI models, indie demos (Spaces), and community-curated daily research papers, Cloudflare is real-world internet traffic stats (trending domains, AI bot adoption, top services per category) — and removing any loses unique signal. Drop "huggingface" only when the founder's domain is clearly non-AI/ML (e.g. trades, brick-and-mortar retail); drop "cloudflare" almost never since deployment-truth signal applies to nearly any tech founder. "producthunt" is in the schema but not yet implemented, so do NOT include it.
