...
import { RedditDeniedError } from './adapters/reddit';
...
type ErrorStatus = Extract<SourceReport['status'], 'timeout' | 'denied' | 'failed'>;
 ...
 * GithubDeniedError, RedditDeniedError, or messages matching
 * rate-limit/denied/429 patterns → denied; everything else → failed.
 * Never throws, so orchestrator can use this in catch blocks without
 * worrying about secondary failures.
 */
export function classifyError(e: unknown): ErrorStatus {
  ...
  if (e instanceof RedditDeniedError) return 'denied';
  if (e instanceof Error) {
    ...
    if (/rate[_ -]?limit/.test(msg) || msg.includes('429') || msg.includes('denied')) {
      return 'denied';
    }
  }
  return 'failed';
